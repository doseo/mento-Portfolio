...
//조회수 업데이트
$result = mysql_query("update noticeboard set view=view+1 where id=$id", $conn);

//글 정보 가져오기
$result = mysql_query("select id,name,email,title,wdate,view,comment,ip from 
                       noticeboard where id=$id", $conn);
...
<!-- 이전 다음 표시 -->
                ...
                // 현재 글보다 id 값이 큰 글 중 가장 작은 것을 가져온다. 즉 바로 이전글
                $query = mysql_query("select id from noticeboard where id > $id limit 1", $conn);
                ...
                $query = mysql_query("select id from noticeboard where id < $id oreder by id desc limit 1", $conn);
